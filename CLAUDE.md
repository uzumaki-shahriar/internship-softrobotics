...
- Run via `docker compose up -d --build` from the repo root (`docker-compose.yml` lives at root, scoped to only this system's app + its own Postgres container — the Gateway and Ecommerce App are not part of it and have/will have their own setup).
...
- **Docker scope:** `docker-compose.yml` at the repo root is for the Bank System only (app + its own Postgres). It is *not* a combined orchestration for all 3 systems — they're separate applications on purpose. If the Gateway or Ecommerce App later get their own Docker setup, that's a separate compose file in their own directory, not an addition to this one.
