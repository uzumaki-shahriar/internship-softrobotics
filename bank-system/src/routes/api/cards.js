...
const { chargeCard, getChargeStatus } = require("../../services/chargeService");
const { refundCard } = require("../../services/refundService");
const { annotateOutcome } = require("../../utils/logOutcome");

const router = express.Router();

router.get("/status/:idempotency_key", async (req, res, next) => {
  try {
    const result = await getChargeStatus(req.params.idempotency_key);
    res.json(result);
  } catch (err) {
    next(err);
  }
